...
tau=0.89
t_decay=eps_strat(2,0.9,0.00001,0.1,device)
print(tau)
...
for eps in range(num_episodes):
  ...
  for i in range(10):
    if(mem.can_provide(64)):
      ...
      ex = policy.forward(next_states.float()).detach().numpy().tolist()
      #print(ex)
      target_q=target_net.forward(next_states.float()).detach().numpy()
      #print(target_q)
      j=[t.index(max(t)) for t in ex]
      
      #print(j)
      #print([target_q[i][j[i]] for i in range(len(j))],j[i])
      #print([target_q[i][j[i]] for i in range(len(j))])
